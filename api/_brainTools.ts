...
export const BRAIN_TOOLS: AiToolDef[] = [
  ...
  },
  {
    ...
    description: 'The player tells you WHERE THE PIN IS on the hole they are playing — "pin\'s back right", "flag is front left today", "it\'s tucked back", "middle pin". RECORD it: every yardage the app gives you from then on plays to that flag (depth), and its side tells you which miss is safe and where to aim. It applies to the hole they are on unless they name another. IN THIS SAME REPLY use it: the numbers this turn do not yet include what they just told you. Front and back are the EDGES of the green — a back pin plays about 5 yards inside the back number, a front pin about 5 past the front number — and aim to the safe side of a side pin. From the next turn the yardage you are given already plays to it. Do NOT call it for a QUESTION ("where\'s the pin?", "is it back right?") — answer that. Call it ALONGSIDE your spoken answer; it never replaces the club call.',
    parameters: {
      type: 'object',
      properties: {
