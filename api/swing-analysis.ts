  ...
  try {
    ...
    if (body.mode === 'locate_swing') {
      ...
      try {
        ...
        const locParts = [
          { text: 'You are an expert golf-swing temporal locator. The golfer INTENTIONALLY pressed record to capture their golf swing, so a swing IS present in these frames — your ONLY job is to identify WHEN it happens, never whether it exists. Using golf-swing knowledge (the motion runs address → takeaway → backswing → top → downswing → impact → follow-through), return the timestamp of IMPACT / the downswing: body rotated toward the target, hips open, arms extended, weight forward, club released — NOT address, waggles, walk-ups, or the settled post-shot pose. CRITICAL — PRACTICE SWINGS vs THE REAL SWING (2026-08-08): clips often contain one or more PRACTICE swings (rehearsals with NO BALL) before the real one. The REAL swing is the one addressed TO A BALL — look for the ball at the player\'s feet at address and gone after impact, a fuller/committed finish, and it is almost always the LAST full swing in the clip. When multiple full swings appear, prefer the LAST one and the one with a visible ball; NEVER return a rehearsal earlier in the clip when a later ball-strike swing exists. You MUST ALWAYS return your best estimate; if you are unsure, choose the frame that most resembles the downswing / impact / immediate follow-through of the FINAL committed swing. Return ONLY JSON: { "swing_time_sec": <number>, "confidence": "high"|"low" }.' },
    ...
    if (body.mode === 'locate_swings') {
      ...
      try {
        ...
        const locParts = [
          { text: 'You are a precise golf-swing temporal locator. List the timestamp of every BALL-STRIKING swing (impact/follow-through moment where a ball is actually hit) in this range-practice video. EXCLUDE: address, waggles, walk-ups, post-shot standing, AND practice/rehearsal swings — a practice swing is a full swing motion made with NO ball being struck (the player is warming up or rehearsing tempo, often beside or short of the ball). Distinguish them: a ball-strike shows a ball at address that departs / a divot / the club bottoming out on the ball line; a practice swing has no ball leaving. IMPORTANT — lean permissive: if you genuinely CANNOT tell whether a ball was struck (e.g. a net catches it instantly, or the ball is out of frame), INCLUDE that swing with confidence "low" rather than dropping it. Only exclude a swing as practice when you are clearly confident no ball was hit. Never report the same swing twice. Return ONLY JSON: { "swings": [ { "time_sec": <number>, "confidence": "high"|"low" } ] } ordered by time. No ball-striking swings visible → { "swings": [] }.' },
          ...locFrames.flatMap((f, i) => {
            const t = typeof f.time_sec === 'number' && Number.isFinite(f.time_sec) ? f.time_sec : i;
            return [
              { text: `Frame ${i + 1} — timestamp ${t.toFixed(1)}s:` },
