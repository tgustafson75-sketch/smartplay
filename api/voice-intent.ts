...
1. open_tool — User wants to launch a tool or screen.
   ...
   - "open issue log" / "show issue log" / "show issues" / "open the issues" / "open bug log" / "show bug log" / "open the issue log" / "let me see the issues" -> { tool_name: "issue_log" }
   - "send issue log" / "email issue log" / "send the issues" / "email the bug log" / "share issue log" / "export issue log" -> { tool_name: "issue_log", send_log: true } (when the verb implies dispatch — send / email / share / export — set send_log:true so the app auto-fires the email export. "show" / "open" / "see" stay as bare open_tool.)
   ...
   CRITICAL — MENTION vs COMMAND (same discipline as the lie camera above): a COMPLAINT, OBSERVATION, or FEEDBACK REQUEST about the swing is NOT an open_tool and must NOT navigate to SwingLab/SmartMotion. Phrases like "we have an issue with my swing" / "something's off with my swing" / "my swing feels wrong" / "give me feedback on my swing" / "what's wrong with my swing" / "help me with my swing" / "let's work on my swing" are the user talking to the CADDIE — classify them as query_status { query_topic: "swing_observation" } (spoken coaching), NOT open_tool. ONLY an EXPLICIT launch verb aimed at the tool opens it: "open/go to/take me to/pull up/launch SwingLab", "record a swing", "start SmartMotion". If the user only MENTIONS the swing without such a verb, do NOT emit open_tool; and if you are unsure whether they want to navigate, emit the open_tool with confidence:"low" so the app OFFERS ("want me to open SwingLab?") instead of yanking them there uninvited.
...
20. sequence — User chained two or more distinct commands in a single utterance, separated by "and", "then", commas, or implicit pause. Each step is a real first-class intent above (open_tool, change_setting, log_shot, etc.). Use this ONLY when the steps are independent actions; do NOT use for a single clause with multiple parameters (e.g. "log driver 240 left" is one log_shot, not a sequence).
   ...
   Examples:
   - "tell ${caddieName} I'm on hole 7, then refresh GPS at the tee" -> { steps: [ { intent_type: "declare_hole", parameters: { hole_number: 7 } }, { intent_type: "refresh_gps", parameters: {} } ] }
   - "log a 5 on this hole and move to the next tee" -> { steps: [ { intent_type: "log_score", parameters: { strokes: 5 } }, { intent_type: "navigate", parameters: { direction: "next_hole" } } ] }
   ...
   - "I'm on in two on hole 1, I went left on my drive with my 5-wood, but we're good to go" -> { steps: [ { intent_type: "declare_hole", parameters: { hole_number: 1 } }, { intent_type: "log_shot", parameters: { club_phrase: "5-wood", outcome_phrase: "left", raw_utterance: "went left on my drive with my 5-wood" } } ] } (extract hole number + shot data from narrative; ignore "on in two" and mood phrases as non-actionable context)
   - "mark my first shot hole 1 in Greenhill" -> { steps: [ { intent_type: "declare_hole", parameters: { hole_number: 1 } }, { intent_type: "log_shot", parameters: { club_phrase: "driver", raw_utterance: "mark my first shot hole 1" } } ] } (first shot = tee shot = driver default; hole number extracted)
   - "I'm on hole 5, hit my 5-wood, went right" -> { steps: [ { intent_type: "declare_hole", parameters: { hole_number: 5 } }, { intent_type: "log_shot", parameters: { club_phrase: "5-wood", outcome_phrase: "right", raw_utterance: "hit my 5-wood, went right" } } ] }
...
8. social_greeting — Greeting or check-in directed at ${caddieName} as a person.
   parameters: {}
   confidence: "high" when the greeting is unambiguous, "medium" otherwise.
   follow_up_question: ALWAYS null. The brain handles it conversationally.
   Examples:
   - "hey ${caddieName}", "hi", "hello", "hey there", "yo"
   - "good morning", "good afternoon", "good evening"
   - "how are you", "how you doing", "what's up", "you there"
...
9. conversational — Free-form question, comment, story, or reflection. NOT a command.
   ...
   follow_up_question: ALWAYS null. The brain handles it conversationally.
   Examples:
   - Golf instruction: "how do I fix my slice", "why does my driver go right", "what is a fade", "tell me about my swing"
   - Trivia / history: "how long has golf had caddies", "who's considered the first caddie", "how many PGA wins does John Daly have", "tell me about Tiger"
   ...
   - Reflections: "rough day", "I'm tired", "this is fun", "let's chat"
