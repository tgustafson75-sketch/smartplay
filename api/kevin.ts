    ...
    const liveFactsBlock = (() => {
      ...
      if (wx) {
        ...
        if (pl && Math.abs(pl.deltaYds) >= 3) {
          ...
          lines.push(`- IT PLAYS ${pl.playsLikeYds}, NOT ${pl.actualYds}${why ? ` (${why})` : ''}. This is measured, not estimated — CLUB TO ${pl.playsLikeYds}. Read down their carries and take the FIRST club AT OR ABOVE ${pl.playsLikeYds} — never one that carries less than ${pl.playsLikeYds} while a longer club is in the bag. Do not talk about the number: he asked what to hit, so give him the club that covers ${pl.playsLikeYds} and at most three words of why ("six — it's into 16").`);
        }
...
Probed 2026-08-24 and got this wrong three times out of three: handed "it plays 209" it answered "six iron, right in your six's wheelhouse" — a club that carries 170, THIRTY-NINE YARDS SHORT, while a 5 wood sat in the bag. Quoting the right number and then naming a club that cannot reach it is worse than never adjusting at all, because they hear you do the clever part and trust the answer. Doing this backwards is the single most common way to be confidently wrong: match the raw number first and the conditions become a remark you tack on instead of the reason for the club. Two things you must never do, because both are confidently wrong and they cannot check you: naming a club that carries well past the number when a nearer one is sitting in the bag, and telling them a club "won't reach" or is "beyond" a number its carry already covers — compare the number to the bag before you say that, every time. Always factor known hazards and doglegs (lay back / take the gap / favor the safe side). Keep it to a club + a one-line why + a confirm.
