...
export class VoiceCommandRouter {
  ...
  async dispatch(intent: VoiceIntent, context: AppContext): Promise<IntentResult> {
    ...
    if (intent.intent_type === 'unknown' || intent.confidence === 'low') {
      ...
      /**
       * 2026-08-23 (Tim's call) — the local-status responder no longer answers here.
       *
       * This fired on classifier unknown/low-confidence, which happens for TWO reasons the code
       * cannot tell apart at this point: the transcript was genuinely free-form (belongs to the
       * brain), or the classifier fetch failed offline. It treated both as local, so free-form
       * questions got a template answer in no persona instead of the caddie.
       *
       * Falling through returns unknown_or_low_confidence and the caller routes to the brain —
       * the path this block's own comment called "the existing brain-fallback path". Offline, the
       * brain call fails and the caller answers honestly; that is a failure handled once, not a
       * second caddie answering first.
       *
       * precheckLocalIntent above STAYS: it matches COMMANDS ("open SmartFinder", "record my
       * swing") and skips a classifier round-trip. Routing a command is not answering a question.
       */
