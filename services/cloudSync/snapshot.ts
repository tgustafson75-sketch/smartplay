 ...
 */
/**
 * 2026-08-12 — persisted stores we DELIBERATELY do not back up, with the reason.
 *
 * This list exists because "a new store was created after the allowlist was curated, so it was
 * silently never backed up" has now happened THREE times — coach-lesson-history and practice-plan on
 * 2026-07-24, and green-reads today, each found only by an audit and each a device-swap data loss in
 * the meantime.
 *
 * An allowlist alone can't catch that: forgetting to add a key looks identical to deciding not to.
 * Pairing it with this list makes the decision EXPLICIT, and the regression test asserts that every
 * persisted store appears in exactly one of the two. A new store then fails the suite until someone
 * states which it is — which is the only way this stops recurring.
 */
export const NOT_BACKED_UP_STORE_KEYS: string[] = [
  // 2026-09-20 — added players' scorecards: other people's names, handicaps and scores. Excluded for
  // the same reason family/guest/relationship/team are, and more strongly: a third party who never
  // installed the app cannot consent to us storing their round on our servers. Also listed in
  // neverSyncKeys so an old blob cannot hand it back. [[automatic-sends-carry-no-pii]]
  'guest-cards-v1',
  // Diagnostics — regenerated by use; restoring stale telemetry would be misleading.
  'swing-analysis-debug-v1',
  'voice-hit-rate-v1',
  'voice-log-v1',
  'voice-misses-v1',
  // UI progress flags — meaningful only on the device that earned them.
  'onboarding-tour-v1',
  'whats-new-v1',
  'session-focus-v1',
  // Device-local caches, re-fetchable and often pointing at local file:// paths.
  'downloaded-courses-v1',
  '@smartplay/reference_authoring',
  // Media-heavy — these hold or reference local file:// clips and base64 blobs that won't survive a
  // reinstall anyway. Deliberately deferred to the phase-2 media sync, per this file's header.
  'cage-store-v1',
  'custom-caddie-media-v1',
  // Device/owner runtime state — meaningless on another handset.
  'capture-engine-v1',
  'gps-health-v1',
  'undo-mark-v1',
...
export const BACKED_UP_STORE_KEYS: string[] = [
  ...
  // events, so the recent-turn window is learned data. Bounded at 60 turns of text, so it costs
  // almost nothing to carry and losing it on a device swap costs the caddie its short-term memory
  // of you. [[self-growing-agent-architecture]]
  'conversation-log',
  ...
  'green-rolls-v1',
  // 2026-08-12 (store sweep) — added, and it's the THIRD time this exact thing has happened: the
  // store was created 08-06, after this allowlist was curated, so it was silently un-backed-up and
  // would have been lost on a device swap. Green reads are player data Tim asked for by name ("the
  // putting read worked fantastic but it doesn't go anywhere — it doesn't get saved"), keyed per
  // course and hole. Structured JSON, no media. See the 07-24 note above for the same story twice.
  'green-reads-v1',
