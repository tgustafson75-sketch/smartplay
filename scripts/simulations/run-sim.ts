...
check('One-time migration clears auto-trapped Local Mode (settings v12)',
  // refreshed: store is at version 17 now (v17 = earbud tap-to-talk on); the one-time
  // version<12 localMode clear is still present (migrations are cumulative), which is
  // what this guards.
  /version: 17/.test(read('store/settingsStore.ts')) &&
