 ...
 * class of bug that left cecilyMode / responseMode / personaIntensity dead).
 */
...
describe('brainSettings — every brain-bound field is forwarded', () => {
  it('exposes exactly the expected keys (guards against a dropped setting)', () => {
    ...
    expect(Object.keys(bs).sort()).toEqual(
      ['aiProvider', 'cecilyMode', 'continuousConversationMode', 'language', 'personaIntensity', 'responseMode'].sort(),
    );
  });
  ...
  it('forwards cecily + response style', () => {
    // 2026-08-25 — tankSoftIntro dropped with the persona; the forwarding behaviour this guards is
    // unchanged for the fields that remain.
    const bs = brainSettings({ caddiePersonality: 'kevin', cecilyMode: true, responseMode: 'short' });
