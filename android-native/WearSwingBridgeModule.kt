        ...
        try {
            Wearable.getNodeClient(reactApplicationContext.applicationContext).connectedNodes
                ...
                .addOnFailureListener { e ->
                    /**
                     * 2026-09-21 — REJECT, do not resolve(0). A FAILED QUERY IS NOT AN ABSENT WATCH.
                     *
                     * This resolved 0, and JS reads `count > 0`, so a Data Layer error rendered in
                     * Settings as "No watch is reachable - check it is paired and nearby" for a
                     * player whose watch was sitting on their wrist, paired and fine. That is the
                     * confident wrong answer the three-state design exists to prevent, and the
                     * comment on this very method says so. Rejecting lands in watchReachable()'s
                     * catch, which returns null = "cannot ask", and the row falls back to the
                     * traffic-derived flag and says nothing new.
                     *
                     * It had to be caught here: this file ships in the binary, so an OTA could
                     * never have corrected it.
                     */
                    Log.w(tag, "connectedNodes query failed (non-fatal): ${e.message}")
                    promise.reject("watch_query_failed", e)
                }
        } catch (t: Throwable) {
            // Same reason as the failure listener above: unknown is not zero.
            Log.w(tag, "getConnectedNodeCount failed (non-fatal)", t)
            promise.reject("watch_query_failed", t)
        }
    }
