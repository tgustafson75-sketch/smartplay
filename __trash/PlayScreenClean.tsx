...
import { getAdvancedPatterns } from '../services/contextBuilder';
...
import { getCaddieAdvice } from '../services/caddieBrain';
...
import { useSwingDetector, type SwingResult } from '../hooks/useSwingDetector';
...
export default function PlayScreenClean() {
  ...
  const [shots, setShots] = useState<Array<{ result: string; timestamp: number; practice: boolean }>>([]);
  const [practiceSession, setPracticeSession] = useState<{ type: string; range?: string } | null>(null);
  ...
  useEffect(() => {
    ...
    if (cal) {
      ...
      setGpsYards({
        front:  Math.abs(hd?.front?.lat  ?? 0) > 0.01 ? safe(haversineYards(c.latitude, c.longitude, hd!.front.lat,  hd!.front.lng))  : null,
        middle: Math.abs(hd?.middle?.lat ?? 0) > 0.01 ? safe(haversineYards(c.latitude, c.longitude, hd!.middle.lat, hd!.middle.lng)) : null,
        back:   Math.abs(hd?.back?.lat   ?? 0) > 0.01 ? safe(haversineYards(c.latitude, c.longitude, hd!.back.lat,   hd!.back.lng))   : null,
      });
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  const startGpsWatch = async () => {
    ...
    try {
      ...
      const computeYards = (lat: number, lng: number) => {
        ...
        const safe = (y: number) => (y > 0 && y <= maxYards ? y : null);
        return {
          front:  Math.abs(hd?.front?.lat  ?? 0) > 0.01 ? safe(haversineYards(lat, lng, hd!.front.lat,  hd!.front.lng))  : null,
          middle: Math.abs(hd?.middle?.lat ?? 0) > 0.01 ? safe(haversineYards(lat, lng, hd!.middle.lat, hd!.middle.lng)) : null,
          back:   Math.abs(hd?.back?.lat   ?? 0) > 0.01 ? safe(haversineYards(lat, lng, hd!.back.lat,   hd!.back.lng))   : null,
        };
      };
  ...
  const getCaddieAdvice = () => {
    if (roundShots.length < 3) return "Let's see a few more swings first.";
  ...
  const getMissBias = (): { bias: string; confidence: number } | null => {
    if (roundShots.length < 5) return null;
    const counts: Record<string, number> = { left: 0, right: 0, straight: 0 };
    roundShots.forEach((s) => { if (s.result in counts) counts[s.result]++; });
    let bias: string | null = null;
    let max = 0;
    Object.entries(counts).forEach(([k, v]) => {
      if (v > max) { max = v; bias = k; }
    });
    return { bias: bias ?? 'straight', confidence: Math.round((max / roundShots.length) * 100) };
  };
  ...
  const getRecentBias = (): 'left' | 'right' | 'straight' | null => {
    const recent = roundShots.slice(-5);
  ...
  const shouldGiveDirectionalAdvice = (): boolean => {
    const confidence = getPatternConfidence(roundShots);
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // speakCaddie: SINGLE CANONICAL VOICE FUNCTION FOR PlayScreenClean
  // Pattern: Immediate acknowledgment + 120ms delay + main response
  // Ensures: no dead silence, consistent pacing, deterministic pipeline
  // ─────────────────────────────────────────────────────────────────────────
  ...
  const getAcknowledgment = (): string => {
    const idx = Math.floor(Math.random() * ACKNOWLEDGMENTS.length);
    return ACKNOWLEDGMENTS[idx];
  };

  const speakCaddie = (text: string, type?: string): void => {
    if (!text || !text.trim() || quietMode || !voiceEnabled) return;
  ...
  const getPreShotMessage = useCallback((): string => {
    const miss = getMissBias()?.bias ?? null;
    const pattern = miss === 'left' ? 'miss_left' : miss === 'right' ? 'miss_right' : 'center';
    const mode = strategyMode === 'attack' ? 'aggressive' : strategyMode === 'safe' ? 'safe' : 'normal';
  ...
  }, [roundShots, distance, shotResult, par, hole, currentHoleData?.note, club, strategyMode]);
  ...
  const runConversation = (_userInput: string): void => {
    // DISABLED: legacy voice output (migrated to V2)
  };
  ...
  const endRound = async () => {
    try {
      ...
      const analysis = computeRoundAnalysis(roundShots, round, roundPars);
      ...
      getRoundInsights(holeStatsLog, roundShots).then((insights) => {
  ...
  const getCaddieResponseV2 = (input: string): string => {
    const text = input.toLowerCase();

    // Practice intent.
    if (text.includes('work on') || text.includes('practice')) {
      if (text.includes('wedge') || text.includes('80')) {
        setPracticeSession({ type: 'wedges', range: '0-80' });
        return 'Perfect-let\'s start around 60 and work down. Just smooth swings.';
      }

      setPracticeSession({ type: 'general' });
      return 'Alright-let\'s get a few swings and see what shows up.';
    }
    ...
    if (text.includes('what should')) {
      return 'I like a 7 here-center is a good line.';
    }

    if (text.includes('watch') || text.includes('record')) {
      return '**START_CAMERA**';
    }
    ...
    if (practiceSession && shots.length >= 3) {
      const last3 = shots.slice(-3);

      const allRight = last3.every((s) => s.result === 'right');
      const allLong = last3.every((s) => s.result === 'long');

      if (allRight) {
        return 'Same miss-let\'s aim a touch left and trust it.';
      }

      if (allLong) {
        return 'You\'re a little long-take a bit off.';
      }
    }

    return 'Play your stock shot.';
  };

  const startCameraRecording = () => {
    console.log('Recording swing...');
  };

  const showOverlay = () => {
    setShowDispersionMap(true);
  };

  const handleUserInput = async (input: string): Promise<void> => {
    if (!input || !input.trim()) return;

    const response = getCaddieResponseV2(input);

    if (response === '**START_CAMERA**') {
      startCameraRecording();
      setCommandResponse('Got it.');
      speakCaddie('Got it.');
      return;
    }
    ...
    if (response === '**SHOW_OVERLAY**') {
      showOverlay();
      setCommandResponse('Here you go.');
      speakCaddie('Here you go.');
      return;
    }

    setLastResponse(response);
    setCommandResponse(response);
    speakCaddie(response);
  };
  ...
  const getDispersion = () => {
    if (roundShots.length < 5) return null;
    const lastTen = roundShots.slice(-10);
    let score = 0;
  ...
  const getContextAdjustment = (): 'safe' | 'neutral' | 'attack' => {
    ...
    const recent = roundShots.slice(-3);
  ...
  const getConfidence = (): number => {
    ...
    let base = roundShots.length < 5
      ? (hasProfileMiss ? 50 : 42)
      : roundShots.length < 10 ? 75 : 90;
    const mb = getMissBias();
  ...
  const speakDecision = (distOverride?: number | string): void => {
    ...
    const advancedPhrase = (roundShots.length >= 5 || cmConfidence >= 30)
      ? buildAdvancedCoachingPhrase()
      : null;
  ...
  const getFullCaddieDecision = () => {
    if (roundShots.length < 5) return coachTone("let's get more swings in first.");
  ...
  const handleCaddie = async () => {
    // DISABLED: legacy voice output (migrated to V2)
    if (caddieCallInFlightRef.current) return;
    caddieCallInFlightRef.current = true;
    setAiThinking(false);
    setIsThinking(false);
    const currentLie = lieType;
    const context = {
      distance: gpsYards?.middle ?? (parseInt(distance, 10) || (currentHoleData?.distance ?? 150)),
      wind: 'into' as const,
      club,
      aim: 'center',
      lie: currentLie || 'fairway',
      practiceSession,
    };
    const advice = USE_CADDIE_V2
      ? getCaddieResponseV2('what should i hit')
      // @ts-expect-error caddieBrain.d.ts type conflicts with ambient-modules.d.ts declaration
      : await getCaddieAdvice({ hole: 1, distance: context.distance, wind: context.wind, shots: roundShots });
    if (typeof advice === 'string') setCaddieMessage(advice);
  ...
  const getDeepCoaching = () => {
    ...
    roundShots.forEach((s) => { if (s.result === 'right') right++; if (s.result === 'left') left++; });
  ...
  const handleVoiceCommand = async (transcript: string) => {
    ...
    // Caddie Brain V2 path (feature-flagged)
    if (USE_CADDIE_V2) {
    ...
    if (calmReply) {
      reply(String(calmReply));
      respond(String(calmReply));
      return;
    }
    ...
    if (
      ...
      if (lower.includes('free relief') || lower.includes('relief')) {
        ...
        if (aiRules != null && `${aiRules}`.length > 0) {
          const safeRules = `${aiRules}`;
          reply(safeRules);
          respond(safeRules);
        } else {
    ...
    if (lower.includes('fix') || lower.includes('how do i fix') || lower.includes('what should i do')) {
      const swingFix = getSwingFixData();
      const fixTitle = swingFix?.title;
      const fixBody = swingFix?.fix;
      if (fixTitle && fixBody) {
        const msg = trimTo12(`${fixTitle}. ${fixBody}.`);
        reply(msg);
        respond(msg);
      } else {
    ...
    if (namedPlayerMatch !== -1 && (lower.includes('got a') || lower.includes('scored a'))) {
      ...
      const parsedScore = parseInt(String(digitMatch?.[1] ?? ''), 10);
      ...
      const finalScore = Number(score ?? NaN);
      if (finalScore > 0 && finalScore <= 15) {
        ...
        setMultiRoundPersisted((prev) => {
          ...
          if (idx === -1) {
            ...
            scores[namedPlayerMatch] = finalScore;
            return [...prev, { hole, par, scores }];
          }
          return prev.map((h, hi) =>
            hi === idx
              ? { ...h, scores: h.scores.map((s, si) => si === namedPlayerMatch ? finalScore : s) }
              : h,
          );
        });
        const msg = `${playerName} scored ${finalScore} on hole ${hole}.`;
        reply(msg);
        respond(msg);
      } else {
        setIsThinking(false);
    ...
    if (lower.includes('got a') || lower.includes('i got a') || lower.includes('scored a')) {
      ...
      const finalScore = Number(score ?? NaN);
      if (finalScore > 0 && finalScore <= 15) {
        setStrokes(finalScore);
        const msg = `Score of ${finalScore} recorded for hole ${hole}.`;
        reply(msg);
        respond(msg);
      } else {
        setIsThinking(false);
  ...
  const getAimInsights = () => {
    if (roundShots.length < 5) return "Not enough data yet ? hit a few more.";
    let mismatchRight = 0;
    let mismatchLeft = 0;
    const lastFive = roundShots.slice(-5);
  ...
  const getSwingFix = () => {
    if (roundShots.length < 5) return "Hit a few shots ? I'll read your pattern soon.";
    let rightMiss = 0;
    let leftMiss = 0;
    roundShots.slice(-10).forEach((shot) => {
  ...
  const getClubInsights = () => {
    if (roundShots.length < 5) return 'Not enough data';
  ...
  const getClubDispersion = () => {
    if (roundShots.length < 6) return 'Not enough club data';
  ...
  const getClubConfidence = () => {
    if (roundShots.length < 6) return 'Not enough data';
  ...
  const getBestClub = () => {
    if (roundShots.length < 6) return null;
  ...
  const getDispersionSpread = () => {
    if (roundShots.length < 5) return null;
    const positions = roundShots.slice(-10).map((shot) =>
  ...
  const getPlayerMemory = () => {
    if (roundShots.length < 10) return 'Building player profile...';
    const recent = roundShots.slice(-10);
    let wideCount = 0;
    let tightCount = 0;
    recent.forEach((shot) => {
  ...
  const getRoundSummary = () => {
    ...
    const trend = getTrend(roundShots);
  ...
  const getPreShotCue = (): string | null => {
    ...
    const { bias } = analyzeShotPattern(roundShots);
    const trend = getTrend(roundShots);
    ...
    return pool[roundShots.length % pool.length];
  };
  ...
  const handleShot = useCallback(async (result: ShotResult) => {
    ...
    setShots((prev) => [...prev, trackedShot]);
  ...
  const decision = useMemo(
    ...
    [roundShots, distance, strategyMode, mentalState, targetDistance, hole, par, gpsYards]
  );
  ...
  const engineDecision = useMemo(() => {
    ...
    const situation = getSituationDecision({
      shots: roundShots.map((s) => ({ result: s.result, hole: s.hole })),
      hole,
      scoreToPar,
      mentalState,
      holesRemaining,
    ...
    return {
      ...getCaddieRecommendation({
        ...
        shots:        roundShots.map((s) => ({ result: s.result })),
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [gpsYards, targetDistance, hole, lieType, mentalState, roundShots, windSpeed, windDir, elevChange, round, roundPars]);
  const caddieAdvice = useMemo(
    () => getShortCaddieDecision(),
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [roundShots, distance, strategyMode, mentalState, gpsYards]
  );
  ...
  const clubStatsMap = useMemo(() => getClubStats(), [roundShots]); // eslint-disable-line react-hooks/exhaustive-deps
  // Memoize the shot-pattern profile for the sim engine — re-runs only when shots change
  const simProfile = useMemo(() => buildSimProfile(roundShots), [roundShots]);
  ...
  return (
    ...
    {quickMode ? (
      <View style={{ flex: 1, backgroundColor: '#0B3D2E', justifyContent: 'center', alignItems: 'center', paddingHorizontal: 32 }}>
        ...
        {!pocketMode && roundShots.length >= 3 && (
    ...
    {!watchMode && !showDetails && !isRoundActive && (
      <View style={{ flex: 1, backgroundColor: highContrast ? '#000' : '#0B3D2E', justifyContent: 'center', alignItems: 'center', paddingHorizontal: 28, paddingBottom: Math.max(tabBarHeight, 16) }}>
        ...
        {(aiRoundInsights || holeStatsLog.length > 0) && (() => {
          const sg = calculateStrokesGained(roundShots);
          return (
      ...
      <View style={{ flex: 1, backgroundColor: highContrast ? '#000' : '#0B3D2E' }}>
        ...
        <View style={{ flex: 1, paddingHorizontal: 16, paddingTop: 12, paddingBottom: tabBarHeight + 12, justifyContent: 'space-between' }}>
          ...
          {(() => {
            ...
            const recentShotsStr = roundShots.slice(-5).map((s) => s.result === 'left' ? '←' : s.result === 'right' ? '→' : '↑').join('  ');
          ...
          {patternExpanded && (() => {
            ...
            const bias = rb ?? mb!.bias;
            const color = bias === 'left' ? '#ef4444' : bias === 'right' ? '#f59e0b' : '#6ee7b7';
            const shotCounts = roundShots.reduce<Record<string,number>>((acc, s) => { acc[s.result] = (acc[s.result] ?? 0) + 1; return acc; }, {});
            const recentArrows = roundShots.slice(-5).map((s) => s.result === 'left' ? '←' : s.result === 'right' ? '→' : '↑').join('  ');
              ...
              onPress={() => {
                ...
                if (hole < roundLength) {
                  ...
                  const keyMsg = finalBias === 'right' ? 'You pushed right all round ? aim left next time.'
                    ...
                    : roundShots.length >= 5 ? 'Straight ball flight ? great consistency!'
    ...
    {!watchMode && showDetails && <ScrollView style={[styles.scroll, { backgroundColor: highContrast ? '#000' : '#0B3D2E' }]} contentContainerStyle={[styles.container, { paddingTop: 48, paddingBottom: tabBarHeight + 16 }]} scrollEnabled={!voiceOverlayActive}>
      ...
      {/* Caddie Brain V2 — practice session status */}
      {(practiceSession || shots.length > 0) && (
        <View style={{ flexDirection: 'row', justifyContent: 'space-between', paddingHorizontal: 4, marginBottom: 4 }}>
          {lastResponse.length > 0 && (
            <Text style={{ color: '#a7f3d0', fontSize: 13, flex: 1 }}>{lastResponse}</Text>
          )}
          <Text style={{ color: '#6ee7b7', fontSize: 13, marginLeft: 8 }}>Shots: {shots.length}</Text>
        </View>
      )}
        ...
        >
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 14 }}>
            ...
            {(() => {
              ...
              const lastResult = roundShots.length > 0 ? roundShots[roundShots.length - 1]?.result : null;
      ...
      <View style={styles.card}>
        ...
        <Pressable onPress={() => setCaddieCollapsed(v => !v)} style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingVertical: 6, marginBottom: caddieCollapsed ? 0 : 10 }}>
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
            ...
            <Pressable onPress={(e) => { e.stopPropagation(); router.push('/tabs/swinglab'); }}
      ...
      <>
        ...
        {showToolsMenu && (
          <View style={{
            ...
            >
              <Text style={{ fontSize: 18 }}>??</Text>
              <Text style={{ color: '#fca5a5', fontSize: 13, fontWeight: '600' }}>Sign Out</Text>
            </Pressable>
      ...
      <View style={{ flex: 1, backgroundColor: '#091410' }}>
        ...
        <WarmupCalibration
          ...
          onComplete={() => {
            ...
            speakCaddie("You're dialled in. Let's go play.", 'confirm');
          }}
        />
      </View>
    </Modal>
